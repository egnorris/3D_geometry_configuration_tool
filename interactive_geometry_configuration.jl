...
for k = 1:length(geometry)
    ...
    T = geometry[k]["thickness"] * config["scaling_factor"]
    L = geometry[k]["length"] * config["scaling_factor"]
    W = geometry[k]["width"] * config["scaling_factor"]
    
    x_slider = Slider(fig[2+(k-1), 1], range = x_domain, startvalue = 50)
    y_slider = Slider(fig[2+(k-1), 2], range = y_domain, startvalue = 50)
    z_slider = Slider(fig[2+(k-1), 3], range = z_domain, startvalue = 50)
    ...
    ax1 = Axis(fig[1, 1], xlabel = "X Slider", title = "XY - Plane")
    poly!(poly_coords_xy)
    ...
    ax3 = Axis(fig[1, 3], xlabel = "Z Slider", title = "YZ - Plane")
    poly!(poly_coords_yz)
